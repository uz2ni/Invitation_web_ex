...
//	    $("#tossLink").append("<a href='" + link + "' class='btn btn-warning' target='_blank'>입금</a>");
	    ...
	    $('#preview_iframe').contents().find('#account-url').attr('href', link);
	  }
	};
...
function accountCreate() {
	let accountName = $('input[name=account-name]').val();
	let accountBirth = $('input[name=account-birth]').val();
	let accountNumber = $('input[name=account-number]').val();
	let accountBank = '003'; // 기업 은행
	
	let d = new Date();
	let tranDtime = "" + d.getFullYear() + "0" + (d.getMonth()+1) + d.getDate() + d.getHours() + d.getMinutes() + d.getSeconds();
	
	console.log('accountName: ' + accountName);
	console.log('accountBirth: ' + accountBirth);
	console.log('accountNumber: ' + accountNumber);
	console.log('accountBanck: ' + accountBank);
	console.log('tranDtime: ' + tranDtime);
...
//            xhr.setRequestHeader("Authorization", "Bearer 08e28d74-664a-4cd8-91da-4a1b38a9aaea");
//        },
        processData: false,
		data: {
			// paymentId빼고 입력
			"Authorization" : "Bearer 08e28d74-664a-4cd8-91da-4a1b38a9aaea",
			"bank_code_std" : accountBank,
			"account_num" : accountNumber,
			"account_holder_info" : accountBirth,
...
function updateAccount() {
	let options = $("select[name=account-select] > option");
	let selectText;
	for(var i=0; i<options.length; i++) {
		if ($(options[i]).is(":selected")==true) {
	...
	// 은행명 추출
	let bankName = selectText.split("[")[1];
	bankName = bankName.split("]")[0];
	// 예금주명 추출
	let accountName = selectText.split("]")[1];
	accountName = accountName.split("(")[0];
	// 계좌 추출
	let accountNumber = selectText.split("(")[1];
	...
	// 송금료
	let accountMoney = $("input[name=account-money]").val();
	
	// toss Url 생성
	createTossUrl(bankName, accountNumber, accountMoney);
	// iframe 내용 변경
	$('#preview_iframe').contents().find('#account-name').text(accountName);
	$('#preview_iframe').contents().find('#account-bank').text(bankName);
	$('#preview_iframe').contents().find('#account-number').text(accountNumberDash);
}